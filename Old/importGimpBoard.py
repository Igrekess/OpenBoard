...
def read_dit_file(dit_path):
    """
    Lire le fichier .dit et extraire les coordonnees des cellules
    Format: index,topLeftX,topLeftY,bottomLeftX,bottomLeftY,bottomRightX,bottomRightY,topRightX,topRightY
    """
    write_log("Reading DIT file: {0}".format(dit_path))
    
    if not os.path.exists(dit_path):
        write_log("ERROR: DIT file not found: {0}".format(dit_path))
        return None
    ...
    try:
                    ...
                    if len(parts) == 2:
                        ...
                        if key == 'overlayFiles':
                            try:
                                ...
                                write_log("Found overlay files in .dit: {0} files".format(len(overlay_files)))
                            except Exception as e:
        ...
        write_log("Successfully read {0} cells from DIT file".format(len(cells)))
        return {
            'cells': cells,
            'metadata': metadata,
        ...
        write_log("ERROR reading DIT file: {0}".format(e))
        return None
...
def extend_board(img, dit_path, cells, metadata, extension_direction, cell_type, overlay_files=None):
    ...
    Parameters:
    - img: L'image GIMP active
    - dit_path: Chemin vers le fichier .dit
    ...
    try:
        ...
        # Mettre a jour le fichier .dit avec les nouvelles cellules
        write_log("Updating .dit file with new cells")
        
        try:
            # Lire le fichier .dit actuel
            with open(dit_path, 'r') as f:
                lines = f.readlines()
            ...
            write_log("Successfully updated .dit file with {0} new cells".format(len(new_cells)))
            
        except Exception as e:
            write_log("ERROR updating .dit file: {0}".format(e))
            return False
...
def import_images_to_board(img, image_files, cell_type, resize_mode, start_cell, auto_extend=False, extension_direction=0, user_overlay_files=None):
    ...
    # Chercher le fichier .dit
    dit_path = None
    if board_path:
        board_dir = os.path.dirname(board_path)
        board_name = os.path.splitext(os.path.basename(board_path))[0]
        dit_path = os.path.join(board_dir, "{0}.dit".format(board_name))
        
        write_log("Looking for DIT file: {0}".format(dit_path))
        
        if not os.path.exists(dit_path):
            write_log("ERROR: DIT file not found: {0}".format(dit_path))
            pdb.gimp_message("DIT file not found. Please open a valid board XCF file.")
            return
    else:
    ...
    if not board_data:
        write_log("ERROR: Failed to read DIT file")
        return
        ...
        for i, image_file in enumerate(image_files):
            ...
            if empty_cell is None and auto_extend:
                ...
                if extension_success:
                    write_log("Board extended successfully, re-reading .dit file")
                    # Relire le fichier .dit pour obtenir les nouvelles cellules
                    board_data = read_dit_file(dit_path)
                    if board_data:
                        cells = board_data['cells']
                        metadata = board_data['metadata']
                        ...
                        write_log("ERROR: Failed to re-read .dit file after extension")
...
def import_board_ui(img, drawable, import_mode, image_folder, image_file, image_pattern, cell_type, 
    ...
    # Les overlays existants du board seront lus depuis le fichier .dit
...
register(
    ...
    "Yan Senez",
    "2025",
    "<Image>/File/Import/Open Board Import...",
    "RGB*, GRAY*",
    [
        # ┌─────────────────────────────────────────────────────────────┐
        # │  📥 IMPORT MODE                                             │
