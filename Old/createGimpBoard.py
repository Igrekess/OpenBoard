...
def write_cell_coordinates(rectangle, dit_path, cell_number):
    """Write cell coordinates to a .dit file"""
    try:
...
def remove_dit_file(file_path):
    """Remove a .dit file if it exists"""
    try:
        if not file_path:
            write_log("No .dit file path provided to remove")
            return False
        
        write_log("Attempting to remove .dit file: {0}".format(file_path))
        
        if os.path.exists(file_path):
            os.remove(file_path)
            write_log(".dit file successfully removed: {0}".format(file_path))
            return True
        else:
            write_log(".dit file does not exist at: {0}".format(file_path))
            return False
    except Exception as e:
        write_log("Error removing .dit file: {0}".format(e))
        return False
...
def write_overlay_metadata_to_dit(dit_path, overlay_files, overlay_indexes):
    """
    Ecrire les metadonnees des overlays dans le fichier .dit
    """
    try:
        ...
        write_log("Overlay metadata written to .dit file")
        return True
    except Exception as e:
...
def create_board_layout(board_name, dest_folder,
    ...
    # Create the .dit file path (same folder as XCF)
    dit_path = os.path.join(dest_folder, "{0}.dit".format(board_name))
    
    write_log("XCF file path: {0}".format(xcf_path))
    write_log("DIT file path: {0}".format(dit_path))
    
    # Remove old .dit file if it exists
    remove_dit_file(dit_path)
    
    # Create new empty .dit file
    try:
    ...
    # Ecrire les metadonnees overlay dans le fichier .dit
    if overlay_files and overlay_indexes:
        try:
            write_log("Writing overlay metadata to .dit file")
...
register(
    ...
    "Yan Senez",
    "2025",
    "<Toolbox>/File/Create/Open Board...",
    "",
    [
        # ════════════════════════════════════════════════════════════════
        # INTERFACE
